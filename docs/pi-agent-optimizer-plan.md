# Pi Agent Local-Model Optimizer — home-lab-mini

**Use Fable 5 (access ends July 7) to select and tune the best local models for home-lab-mini's hardware and the Pi Agent harness — reachable over Tailscale SSH, wired into Hermes and the Organized Fable local tier.**
...
- **Target:** `home-lab-mini` · Tailscale `<tailscale-ip>` (16GB M-series; runs Hermes + OpenClaw + local render)
- **Control machine:** control-laptop (M1 Pro) -> Tailscale SSH -> home-lab-mini
...
- **Phase 0 - Access + deploy:** confirm Tailscale SSH to home-lab-mini; deploy the optimizer surface (Worker Assets + GSAP) on the fable-cal project; `pi_models`/`pi_tasks` D1 tables (done).
- **Phase 1 - Inventory:** SSH-profile home-lab-mini (RAM/Metal/thermal/disk/runtimes); enumerate the Pi Agent task taxonomy (hygiene, coding, routing, embeddings) with SLAs.
...
- **Phase 6 - Nightly hygiene:** gemma-class LaunchAgent on home-lab-mini (digest/cleanup) -> Fable ledger.
